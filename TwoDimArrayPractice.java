...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int intermedResult = intArray[0][column];
	   for (int row = 0; row < intArray.length; row++) {
		   if (intArray[row][column]<intermedResult) {
			   intermedResult=intArray[row][column];
		   }
		   animate(row, column, intermedResult);
	   }

      return intermedResult; // replace this line with your return statement
   }
